...
class PasswordManager:
        ...
        if self.records_count != 0:
            ...
            for _ in range(3):
                try:
                    ...
                    print("\U0000274C Invalid record")
                    continue
    ...
    def change_db_password(self):
        while True:
            ...
            new_password = getpass("New decryption key: ")
            if len(new_password) < 10:
                print("\U0000274C Password must be at least 10 characters")
                continue

            confirm_new_password = getpass("Confirm new decryption key: ")
        ...
        if self.records_count != 0:
            for _ in range(3):
                ...
                if self.db_key_hash == decryption_key_hash:
                    ...
                    print(f"\U00002714 database backup saved in {os.getcwd() + '/passwords.db.bak'}")
                    break
                else:
                    print("\U0000274C incorrect database decryption key ")
        else:
            print("\U0001F5D1 no records backed up")
